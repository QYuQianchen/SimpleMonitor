...
function linkDevices(_config) {
  ...
  // linking battery0 with all the houses, PVs and HPs
  for (let i = 0; i < 3; i++) {
    console.log("Linking house[" + i + "] with battery0");
    linkDevicesPromises.push(configuration.linkDevices(_config.house[i].address,  _config.battery[0].address, { from: _config.admin[0].address, gas: 2000000 }));
    console.log("Linking pv[" + i + "] with battery0");
    linkDevicesPromises.push(configuration.linkDevices(_config.pv[i].address, _config.battery[0].address, { from: _config.admin[0].address, gas: 2000000 }));  
    console.log("Linking heatpump[" + i + "] with battery0");
    linkDevicesPromises.push(configuration.linkDevices(_config.heatpump[i].address, _config.battery[0].address, { from: _config.admin[0].address, gas: 2000000 }));
  }
