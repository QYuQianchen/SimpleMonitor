...
// CHECK COLLECTED PROMISES
  ...
  it("II. Set Production and price", function() {
    ...
    return checkStep().then(function(result){
      ...
      console.log("Set two houses consumption.");
      ...
      for (device_type in config) {
        for (device_id in config[device_type]) {
          (function(element) {
            ...
            console.log("The consumption of",element.device_type,"is ",result[0].toNumber(),result[1].toNumber());
            }));
          }
        })(config[device_type][device_id]);
        }
      ...
      console.log("get all values via loop and function");
