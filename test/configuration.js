...
contract('Configuration', function(accounts) {
  ...
  it("Create 3 SingleHouse contracts and link to 3 SinglePVs", function() {
    ...
    return Configuration.deployed().then(function(instance){
      ...
      configuration.addBattery(address_B0,20, true);
  ...
  it("Price communication House<->PV (1. House ask for price info)", function() {
    ...
    /*  
      ...
      singleBattery0.sort();
      return singleBattery0.getSortedPVInfo.call({from: singlePV0_adr});
    }).then(function(result){
      console.log("Battery 0 asked and sorted, PV0 is in",result[0].toNumber(),result[1].toNumber(),result[2].toNumber(),result[3]);
      return singleBattery0.getSortedPVInfo.call({from: grid_adr});
    }).then(function(result){
      console.log("Battery 0 asked and sorted, grid is in",result[0].toNumber(),result[1].toNumber(),result[2].toNumber(),result[3]);
      return singleBattery0.getBuyVol.call();
    }).then(function(result){
      console.log("Battery 0 wants to buy", result.toNumber());
...
/*
    singlePV1.initiateTransaction(0).then(function(result){
      ...
      console.log("Now B0 wants",result.toNumber());*/
    });
  });
